...
export function ProductLandingPage({ product, marketing, related }: Props) {
  ...
  return (
    <div className="pb-28">
      ...
      <section className="px-4 py-6 sm:px-6">
        <div className="mx-auto max-w-4xl space-y-6">
          ...
          <div className="space-y-4">
            <h1 className="text-2xl font-bold leading-tight text-brand-text sm:text-3xl">
              {marketing.painHeadline}
            </h1>
            <p className="text-base leading-8 text-brand-muted">{marketing.painSubheadline}</p>
          </div>
          ...
          <p className="text-sm font-medium text-brand-accent">{marketing.scarcity}</p>
          <p className="text-xl font-bold">{formatPriceFrom(selectedOffer.price)}</p>
          ...
          <button
            type="button"
            onClick={handleCta}
            className="w-full rounded-full bg-brand-primary py-4 text-lg font-bold text-white shadow-luxury"
          >
      ...
      <ProductStickyCTA
        label={marketing.stickyCtaTemplate}
